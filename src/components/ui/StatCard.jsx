...
export default function StatCard({ icon, value, label, variant = 'primary', trend, className = '', onClick }) {
  ...
  return (
    <div 
      onClick={onClick}
      className={`relative overflow-hidden ${bgClass} rounded-2xl text-white p-5 shadow-sm transition-transform hover:-translate-y-1 duration-300 border border-white/15 font-['Roboto'] ${onClick ? 'cursor-pointer' : ''} ${className}`}
    >
      ...
      <div className="relative z-10">
        <div className="text-3xl md:text-4xl font-extrabold tracking-tight font-['Roboto']">{value}</div>
